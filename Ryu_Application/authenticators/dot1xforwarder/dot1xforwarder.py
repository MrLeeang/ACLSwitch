...
class DpList(object):
    ...
    def new_client(self, mac, user):
        ...
        for d in self.datapaths:
            ...
            actions = parser.OFPInstructionGotoTable(self._l2switch_table)
            inst = [actions]
            match = parser.OFPMatch(eth_src=mac, eth_type=Proto.ETHER_IP, ip_proto=Proto.IP_UDP,
                                    udp_dst=Proto.DHCP_SERVER_DST)
            self._contr.add_flow(d, 5200, match, inst, 0, self._table_id_1x, cookie=0x20)

            match = parser.OFPMatch(eth_dst=mac, eth_type=Proto.ETHER_IP, ip_proto=Proto.IP_UDP,
                                    udp_dst=Proto.DHCP_CLIENT_DST)
...
class Dot1XForwarder(ABCRyuApp):
    ...
    def packet_in(self, event):
        ...
        if event.msg.table_id != self._table_id_1x:
            self._logging.debug("ignore packet from other tables %d, my table: %d", event.msg.table_id,
                                self._table_id_1x)
            return
